...
public class UserProfileDAOTest {
	...
	@Inject private UserProfileDAO userProfileDAO;
	...
	public void createUpdateAndDeleteTest() {
		...
		assert(roleDAO.count() == 1);
		assert(userAccountDAO.count() == 1);
		assert(userProfileDAO.count() == 0);
		long counter = userProfileDAO.count();
		UserProfile anUserProfile = new UserProfile();
		anUserProfile.setUserAccount(userAccount);
		anUserProfile.setFirstName("John");
		anUserProfile.setMiddleName("D.");
		anUserProfile.setLastName("Doe");
		anUserProfile.setEmailRef("amanager@sampleemailaddress.com");
		anUserProfile.setEmployer("Evil Inc.");
		anUserProfile.setNationality("IT");
		anUserProfile.setTaxcode("1234ABCD5678EFGH");
		counter++;
		UserProfile createdUserProfile = userProfileDAO.create(anUserProfile);
		Assert.assertEquals(counter, userProfileDAO.count());
		createdUserProfile.setWorkPhone("+1 123 45678 00");
		UserProfile updatedUserProfile = userProfileDAO.update(createdUserProfile);
		Assert.assertEquals("+1 123 45678 00", updatedUserProfile.getWorkPhone());
		userProfileDAO.delete(updatedUserProfile.getUserAccountId());
		counter--;
		Assert.assertEquals(counter, userProfileDAO.count());
		assert(userProfileDAO.count() == 0);
	}
