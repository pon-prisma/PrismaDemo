 ...
 * 
 * @author m.grandolfo
 * This class wraps the API from Zabbix platform
 */
@Stateless
public class WrapperWatcher implements Serializable{
	...
	public MonitoringWrappedResponsePaas getWrappedWatcher(
			String testbedType, 
			String url, 
			String token, 
			String workGroup,
	...
	private ArrayList<Group> getGroupInfo(
			String testbedType,
			String url,
			String token,
			String group,
		...
		List<ZabbixHostGroupResponse> workgroupInfo = groupInfo;

		ArrayList<Group> groupsList = new ArrayList<>();
		boolean workgroupFound = false;
		String workgrupName = null;
		...
		for (i=0; i<workgroupInfo.size(); i++) {  
			...
			workgrupName = workgroupInfo.get(i).getName();
			workGroup.setGroupName(workgrupName);

			if(group.equalsIgnoreCase(workgrupName)) { 
				workgroupFound=true;
				 ...
				 * Metric Specified
				 ******************/
				if(!(metrics_id==null) && triggers_id==null && history==null)
				{	
					...
					Iterator<ZabbixMonitoredHostResponseV2_4> hostIt = hostInfo.iterator();
					boolean hostFound=false; 
					while(!hostFound && hostIt.hasNext())
					{
						ZabbixMonitoredHostResponseV2_4 adapterResult = hostIt.next();
					...
					if(hostFound==false && !(hostIt.hasNext()))
				...
				{   
					...
					for(ZabbixMonitoredHostResponseV2_4 hostResult : hostInfo){
						hostName = hostResult.getName();
						//							if(hostName.equals(decodedPaasKey[1]))
		...
		if (workgroupFound==false && i==workgroupInfo.size() && !(group.equalsIgnoreCase(workgrupName)) ){
	...
	private ArrayList<Service> getAtomicService(
			String testbedType,
			String url,
			String token,
			String hostIdtoAdapter, 
	 ...
	 */
	@SuppressWarnings({ "unchecked", "unused" })
	private List<PaaSMetric> getPaasMetrics( 
			String testbedType,
			String url,
			String token,
			String hostidtoAdapter,
		...
		{	
			...
			for(ZabbixItemResponse metrictype : metrics4Host){
				...
				Float metricValue = null;
//				String metricInstantTime = timeConv.decodUnixTime2Date(Long.parseLong(metrictype.getLastclock()));
				if( metrictype.getLastvalue()!=null && !(metrictype.getLastvalue().equals(""))  ){
					metricValue = MetricsParserHelper.getMetricParsedValue("0",metrictype.getValue());
				}
				else metricValue = (float) -1;
...
//				paasMetrics.setMetricTime(clock4Graph.get(0));
	 ...
	 */
	@SuppressWarnings("unchecked")
	private ArrayList<PaasThreshold> getThreshold(
			String testbedType,
			String url,
			String token,
			String hostIdtoAdapter,
